...
from .models import Invoice, Quote, Ticket, TicketEvent
...
def auto_assign(ticket: Ticket):
    ...
    managers = Membership.objects.filter(
        company=ticket.company, role="MANAGER"
    ).select_related("user")
    adminz = Membership.objects.filter(
    ...
    if target and (ticket.assigned_to.id if ticket.assigned_to else None) != target.id:
        ...
        log_event(
            ticket,
            "ASSIGNED",
            f"Ticket attribué automatiquement à {target}",
            actor=None,
        )


def next_order(company, status):
    m = Ticket.objects.filter(company=company, status=status).aggregate(m=Max("order"))[
        "m"
    ]
    return (m or 0) + 1
...
def next_quote_number(company):
    ...
    prefix = f"DEV-{y}-"
    last = (
        Quote.objects.filter(company=company, number__startswith=prefix)
        .order_by("-number")
        .first()
    )
...
def next_invoice_number(company):
    ...
    prefix = f"FAC-{y}-"
    last = (
        Invoice.objects.filter(company=company, number__startswith=prefix)
        .order_by("-number")
        .first()
    )
    seq = int(last.number.split("-")[-1]) + 1 if last else 1
