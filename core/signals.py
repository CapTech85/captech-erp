...
from .models import Membership, Ticket, TicketAttachment, TicketComment
from .services import auto_assign, log_event, next_order
...
def _company_recipients(ticket: Ticket):
    ...
    admins = Membership.objects.filter(
        company=ticket.company, role="ADMIN"
    ).select_related("user")
    for m in admins:
        if m.user.email:
...
def on_comment(sender, instance: TicketComment, created, **kwargs):
    if created:
        log_event(
            instance.ticket,
            "COMMENT_ADDED",
            f"Commentaire ajouté par {instance.author}: {instance.message[:120]}",
...
def on_attachment(sender, instance: TicketAttachment, created, **kwargs):
    if created:
        log_event(
            instance.ticket,
            "ATTACHMENT_ADDED",
            f"Pièce jointe ajoutée: {instance.file.name}",
