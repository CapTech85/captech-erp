...
class CompanyBoundAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        if request.user.is_superuser or request.user.is_staff:
            ...
            # si superuser sans grant, tu peux autoriser
            # tout en commentant la ligne suivante
            return (
                qs.filter(company__in=allowed)
                if qs.model != models.Company
                else allowed
            )
        return qs.none()
