...
class Company(Timestamped):
    ...
    legal_status = models.CharField(
        max_length=20, choices=LEGAL_STATUS, default="MICRO"
    )
    urssaf_frequency = models.CharField(
        max_length=12, choices=URSSAF_FREQ, default="TRIMESTRIEL"
    )
    activity_kind = models.CharField(
        max_length=20, choices=ACTIVITY_KIND, default="SERVICES_BIC"
    )
...
class TurnoverEntry(models.Model):
    ...
    company = models.ForeignKey(
        Company, related_name="turnover_entries", on_delete=models.CASCADE
    )
...
class LegalThresholds(models.Model):
    ...
    micro_cap_sales = models.PositiveIntegerField(
        default=188700
    )  # €  :contentReference[oaicite:4]{index=4}
    micro_cap_services = models.PositiveIntegerField(
        default=77700
    )  # €  :contentReference[oaicite:5]{index=5}
    # Franchise TVA (Service-Public 2025, mesure 25k € suspendue fin 2025)
    vat_base_sales = models.PositiveIntegerField(
        default=85000
    )  # €  :contentReference[oaicite:6]{index=6}
    vat_base_sales_tol = models.PositiveIntegerField(
        default=93500
    )  # €  :contentReference[oaicite:7]{index=7}
    vat_base_services = models.PositiveIntegerField(
        default=37500
    )  # €  :contentReference[oaicite:8]{index=8}
    vat_base_services_tol = models.PositiveIntegerField(
        default=41250
...
class Ticket(Timestamped):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="tickets"
    )
...
class Quote(models.Model):
    ...
    company = models.ForeignKey(
        "Company", on_delete=models.CASCADE, related_name="quotes"
    )
...
class SupportAccessGrant(models.Model):
    ...
    company = models.ForeignKey(
        "Company", on_delete=models.CASCADE, related_name="support_grants"
    )
    granted_to = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, related_name="support_grants"
    )
    granted_by = models.ForeignKey(
        "auth.User",
        on_delete=models.SET_NULL,
        null=True,
        related_name="support_grants_by",
    )
