...
def _fake_quote(pk: int):
    return {
        ...
        "customer": {
            "name": "Prospect Démo",
            "billing_address": "2 avenue Test\n69000 Lyon",
            "vat_number": "",
        },
        "items": [
            {
                "description": "Mission découverte",
                "quantity": 1,
            ...
            {
                "description": "Atelier cadrage (jour)",
                "quantity": 2,
...
def _fake_urssaf(pk: int):
    ...
    periods = [
        {
            "label": "T1 2025",
            "turnover_cents": 1500000,
            "expenses_cents": 200000,
            "rate_pct": 22.0,
        },
        {
            "label": "T2 2025",
            "turnover_cents": 1200000,
            "expenses_cents": 150000,
            "rate_pct": 22.0,
        },
        {
            "label": "T3 2025",
            "turnover_cents": 1750000,
            "expenses_cents": 180000,
            "rate_pct": 22.0,
        },
        {
            "label": "T4 2025",
            "turnover_cents": 1100000,
...
def build_urssaf_context(raw):
    ...
    for p in raw["periods"]:
        ...
        taxable = money(
            turnover - expenses if turnover > expenses else turnover
        )  # simplif micro: frais non déductibles, ajuste selon régime
...
def render_pdf(template_name: str, context: dict, filename: str) -> HttpResponse:
    ...
    pisa_status = pisa.CreatePDF(
        html, dest=resp, link_callback=link_callback, encoding="utf-8"
    )
    if pisa_status.err:
